...
export default function Inbox() {
  const [candidates, setCandidates] = useState([
    { id: 1, title: '체험학습 CMS 출금 안내', source: '직접 입력', date: '확인 필요', importance: 'important' },
    { id: 2, title: '정보보안 교육 이수', source: '직접 입력', date: '7월 5일', importance: 'normal' }
  ]);
    ...
    <div>
      <div style={{ marginBottom: '24px', display: 'flex', alignItems: 'center', gap: '12px' }}>
        <div>
          ...
          <p className="text-caption" style={{ marginTop: '6px' }}>직접 입력된 업무 메시지입니다.</p>
        </div>
      </div>
      ...
      <div style={{ borderBottom: '1px solid var(--color-hairline)', marginBottom: '24px', display: 'flex', gap: '24px' }}>
        {tabs.map((tab, idx) => (
          <button 
            key={idx} 
            onClick={() => { /* optional: implement tab switching if needed */ }}
            style={{ 
              paddingBottom: '12px', 
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: '16px' }}>
        ...
        {saved.length === 0 && <p style={{ color: 'var(--color-muted-ink)' }}>저장한 업무가 없습니다.</p>}
        {saved.map(task => (
          <div key={task.id} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            <h4 className="text-body-strong">{task.title}</h4>
        ...
        {ignored.length === 0 && <p style={{ color: 'var(--color-muted-ink)' }}>무시한 메시지가 없습니다.</p>}
        {ignored.map(task => (
